...
def main():
    ...
    try:
        raw_rounds = input(
            "Enter number of rounds to play (odd number recommended) or press Enter to play until you quit: "
        ).strip()
        if raw_rounds:
    ...
    try:
        while True:
            ...
            if user is None:
                print(
                    "I didn't understand that — please enter rock, paper, scissors, or q to quit."
                )
                round_no -= 1
                continue
            if user == "quit":
